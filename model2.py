...
class EncoderCNN(nn.Module):
    def __init__(self, embed_size, layer8 = False):
        ...
        # self.bn = nn.BatchNorm1d(embed_size, momentum=0.01)
            ...
            for i,child in enumerate(self.resnet.children()):
                ...
                for param in child.parameters():
                    param.requires_grad = False
            # modules1 = list(resnet.children())[:-3]      # delete the last fc layer.
            # modules2 = list(resnet.children())[-3:-1] 
...
class DecoderRNN(nn.Module):
    def __init__(self, embed_size, hidden_size, vocab_size, num_layers, dropout = None, max_seq_length=20):
    ...
    def forward(self, features, captions, lengths, train = True):
        ...
        if not self.dropout or train == False:
        ...
        elif self.dropout and train:
            _, state = self.lstm(features.unsqueeze(1))
            embeddings = self.embed(captions)           # (batch_size, lengths[0], embed_size)
            _, pack_size = pack_padded_sequence(embeddings, lengths, batch_first=True)
            #x.data.new(x.size()).bernoulli_(1 - dropout)
            outputs = []
            #if train:
            mask = torch.autograd.Variable(
                embeddings.data.new(features.size(0),self.hidden_size).bernoulli_(1 - self.dropout), 
                requires_grad=False) / (1 - self.dropout)       # (batch_size, hidden_size)
            for t in range(lengths[0]):
                hiddens, state = self.lstm(embeddings[:pack_size[t].item(),[t],:], 
                    (state[0][:,:pack_size[t].item(),:],state[1][:,:pack_size[t].item(),:]))
                
                # (hiddens.squeeze(1) * mask)
                outputs.extend(self.linear(hiddens.squeeze(1) * mask[:pack_size[t].item(),:]))
